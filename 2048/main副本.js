	...
	init : function() {
		...
		game.States.preload = function() {
			this.preload = function() {
				...
				game.load.image("16","assets/images/16.png");
				game.load.image("32","assets/images/32.png");
				game.load.image("64","assets/images/64.png");
				game.load.image("128","assets/images/128.png");
				game.load.image("256","assets/images/256.png");
				game.load.image("512","assets/images/512.png");
				game.load.image("1024","assets/images/1024.png");
				game.load.image("2048","assets/images/2048.png");
				game.load.image("bg_box","assets/images/bg_box.png");

				//加载音乐资源
				game.load.audio("bgm","assets/audio/bgm.mp3");
				game.load.audio("tap","assets/audio/tap_music.mp3");
				game.load.audio("level","assets/audio/level_music.mp3");
		...
		game.States.create = function() {
			this.create = function() {
				...
				// 创建音乐管理器
				self.musicManager = new MusicManager(game, self.device, ['bgm','tap','level']);
				// 显示开始菜单页面 使用dom构建
				// $('#start-menu').show();
			}
		};
		...
		game.States.play = function(){
			this.create = function(){
				...
				side = block_width;
				var bg_temp=game.add.image(0,0,"bg_box");
				...
				for (i = 0; i < 4; i++) {
					for (j = 0; j < 4; j++) {
						...
						var bg_img=game.add.image(x, y, "bg_box");
				...
				$('body').on('swipeDown', function() {
					if (swipeLock){
						self.musicManager.play("tap", false);
						swipeLock = false;
						var move_to;
						can_update = false;
						for (j = 0; j < 4; j++) {
					...
					if (flag) {
						...
						tween.onComplete.add(function() {
							...
							if(value>level_value){
								self.musicManager.play("level", false);
